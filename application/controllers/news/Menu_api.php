...

    {
        ...
        if (!isset($_GET['act'])) {
            ...
            if ($_GET['act'] == 'load') {
                ...
                $queryDataRead = $this->crud_function_model->selectJointhreeTable('c.`nama_menu`,c.`link`,c.`uniq_menu`,c.`menu_utama_id`, a.`create`,a.`read`,a.`update`,a.`delete`, c.`status`', 'menu_access a','user b', 'a.`user_id` = b.`uniq_user`','', 'menu c', 'a.`menu_id` = c.`uniq_menu`', '', 'a.`user_id` = '.$_SESSION['uniq_user'].'', 'c.nama_menu ASC');

                foreach ($queryDataRead as $item) {

                    // memfilter akses yang di berikan

                    if($item['create'] == null || $item['create'] == "" and $item['read'] == null || $item['read'] == "" and $item['update'] == null || $item['update'] == "" and $item['delete'] == null || $item['delete'] == "" and $item['status'] == 2){
