...

{
    ...
    public function home(){
        ...
        $data = array(
            ...
            'title' => 'indonesiamengabari.com',

            'content_desk' => 'Media Nasional Berjaringan indonesiamengabari.com adalah media mainstream yang sudah memiliki legalitas resmi Kemenkumham dan Terverifikasi Faktual Dewan Pers',

            'url_segment' => $urlpages,

            'link_page' => current_url(),

            'title_url' =>  str_replace("-", " ",$this->uri->segment(3)).'indonesia mengabari'
    ...
    public function content_data(){
        ...
        $wherePengunjung = array(

            'a.berita_id' => $_GET['uniq_berita']

        );

        $pengunjungQuery = $this->crud_function_model->readData("pengunjung a", "a.jumlah", $wherePengunjung, "", "");

        foreach($pengunjungQuery as $pengunjungItem);
        ...
        if(empty($pengunjungItem)){

            $pengunjungData = "0";

        } else {

            $pengunjungData = $pengunjungItem;
    ...
    public function pengunjung_proses($typeProses, $dataPengunjung, $uniqBerita){

        if($typeProses == 'insert'){

            $datainsert = array(

                'uniq_pengunjung' => 'uniq-'.$uniqBerita,

                'berita_id' => $uniqBerita,

                'jumlah' => $dataPengunjung + 1

            );

            $this->crud_function_model->insertData2('pengunjung',  $datainsert);

        } else if($typeProses == 'update'){
            ...
            $this->crud_function_model->updateData('pengunjung', $setUpdate, $whereUpdate);
